...
def get_activations(test_loader, device, model, save=False, save_as=""):
    ...
    if save == True:
        ...
        th.save(
            ...
            f"activations/hidden_activ_{save_as}.pt",
        )
        th.save(
            output_activations_torch,
            f"activations/output_activ_{save_as}.pt",
        )

        th.save(
            predicted_torch,
            f"predicted/{save_as}.pt",
        )

    return hidden_activations, output_activations, predicted


def get_weights(model, save=False, save_as=""):
    ...
    if save == True:
        ...
        th.save(
            weights_first_layer,
            f"weights/weights_first_layer_{save_as}.pt",
        )
        th.save(
            weights_second_layer,
            f"weights/weights_second_layer_{save_as}.pt",
        )
