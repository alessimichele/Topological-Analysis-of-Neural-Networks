...
def compute_mapper(
    data, title="", save_as="", n_cubes=1, clusterer=KMeans(n_clusters=10), cube=0
):
    ...
    graph = mapper.map(
        projected_data,
        data,
        cover=km.Cover(n_cubes=n_cubes, perc_overlap=0.5),
        clusterer=clusterer,
    )
    ...
    mapper.visualize(
        graph,
        path_html=f"plot_outputs/mapper/mapper_outputs/{save_as}.html",
        title=title,
    )
